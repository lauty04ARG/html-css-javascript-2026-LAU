...
- [x] **REQ-1.1.1** La página tiene una estructura HTML5 válida (`<!DOCTYPE html>`, `<html lang="es">`, `<head>`, `<body>`)
- [x] **REQ-1.1.2** El `<head>` incluye: `charset` UTF-8, `viewport` meta tag, `<title>` descriptivo, y al menos un `<meta name="description">`
- [x] **REQ-1.1.3** Existe exactamente un `<header>` de página que contiene: logo (imagen con `alt` descriptivo) y una navegación (`<nav>`) con al menos 4 enlaces (`<a>`)
- [x] **REQ-1.1.4** El contenido principal está dentro de `<main>`
